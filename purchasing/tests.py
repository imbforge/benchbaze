...
class OrderAPITest(APITestCase):
    ...
    @skip(
        "Order has no _obj_specific_fields so the generic create endpoint cannot accept field data; creation must go through the admin interface."
    )
    ...
    @skip(
        "Order has no _obj_specific_fields so the generic PATCH endpoint does not persist field changes; updates must go through the admin interface."
    )
