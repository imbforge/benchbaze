...
def _make_species(latin_name="Homo sapiens", **kwargs):
    defaults = {"latin_name": latin_name, "risk_group": 1}
...
class SpeciesModelTest(TestCase):
    ...
    def test_str_without_risk_group(self):
        s = Species.objects.create(common_name="No risk group", risk_group=None)
    ...
    def test_display_name_returns_common_name_when_no_latin(self):
        s = Species.objects.create(common_name="Mouse", risk_group=1)
        self.assertEqual(s.display_name, "Mouse")

    @skip("name_for_search field no longer exists in Species model")
    ...
    def test_clean_raises_when_neither_name_set(self):
        s = Species(risk_group=1)
        with self.assertRaises(ValidationError):
            s.clean()

    def test_clean_passes_with_latin_name(self):
        s = Species(latin_name="Caenorhabditis elegans", risk_group=1)
        try:
            s.clean()
    ...
    def test_clean_passes_with_common_name_only(self):
        s = Species(common_name="Nematode", risk_group=1)
        try:
            s.clean()
        except ValidationError:
            self.fail("clean() raised unexpectedly")

    def test_clean_raises_on_duplicate_unified_name_when_updating(self):
        _make_species(latin_name="Streptomyces scabiei", risk_group=1)
        existing = _make_species(latin_name="Other thing", risk_group=1)
        existing.latin_name = "Streptomyces scabiei"
        with self.assertRaises(ValidationError):
            existing.clean()

    def test_clean_raises_on_duplicate_latin_name_when_both_names_are_set(self):
        _make_species(latin_name="Streptomyces scabiei", risk_group=1)
        existing = _make_species(
            latin_name="Other thing", common_name="Other thing", risk_group=1
        )
        existing.latin_name = "Streptomyces scabiei"
        existing.common_name = "Something else"
        with self.assertRaises(ValidationError):
            existing.clean()
...
class SequenceFeatureModelTest(TestCase):
    ...
    def test_str_non_common_feature_includes_species(self):
        species = _make_species(latin_name="Aequorea victoria")
    ...
    def test_str_non_common_feature_without_donor_returns_name(self):
        feat = _make_sequence_feature(name="NoDonor", common_feature=False)
        self.assertEqual(str(feat), "NoDonor")
    ...
    def test_get_donor_species_risk_groups_empty_when_no_donor(self):
        self.assertEqual(self.feat.donor_species_risk_groups(), "")

    def test_get_donor_species_max_risk_group_zero_when_no_donor(self):
        self.assertEqual(self.feat.donor_species_max_risk_group(), 0)

    def test_get_donor_species_risk_groups_with_donor(self):
        species = _make_species(latin_name="Escherichia coli", risk_group=1)
        self.feat.donor_organism.add(species)
        self.assertEqual(self.feat.donor_species_risk_groups(), "1")

    def test_get_donor_species_max_risk_group_with_donor(self):
        species = _make_species(latin_name="Escherichia coli K12", risk_group=1)
        self.feat.donor_organism.add(species)
