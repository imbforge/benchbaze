...
describe("test DjangoQL completion", () => {
  ...
  describe(".lexer", () => {
    ...
    it("should recognize reserved words", () => {
      ...
      words.forEach((word) => {
        expect(djangoQL.lexer.lex()).toStrictEqual(
          token(word.toUpperCase(), word)
        );
      });
    });
    ...
    it("should recognize strings", () => {
      ...
      strings.forEach((s) => {
        expect(djangoQL.lexer.lex()).toStrictEqual(
          token("STRING_VALUE", s.slice(1, s.length - 1))
        );
      });
    });
  ...
  describe(".resolveName()", () => {
    it("should properly resolve known names", () => {
      expect(djangoQL.resolveName("price")).toStrictEqual({
        model: "core.book",
        field: "price",
        modelStack: ["core.book"]
      });
      expect(djangoQL.resolveName("author")).toStrictEqual({
        model: "auth.user",
        field: null,
        modelStack: ["core.book", "auth.user"]
      });
      expect(djangoQL.resolveName("author.first_name")).toStrictEqual({
        model: "auth.user",
        field: "first_name",
        modelStack: ["core.book", "auth.user"]
      });
      expect(djangoQL.resolveName("author.groups")).toStrictEqual({
        model: "auth.group",
        field: null,
        modelStack: ["core.book", "auth.user", "auth.group"]
      });
      expect(djangoQL.resolveName("author.groups.id")).toStrictEqual({
        model: "auth.group",
        field: "id",
        modelStack: ["core.book", "auth.user", "auth.group"]
      });
      expect(djangoQL.resolveName("author.groups.user")).toStrictEqual({
        model: "auth.user",
        field: null,
        modelStack: ["core.book", "auth.user", "auth.group", "auth.user"]
      });
      expect(djangoQL.resolveName("author.groups.user.email")).toStrictEqual({
        model: "auth.user",
        field: "email",
        modelStack: ["core.book", "auth.user", "auth.group", "auth.user"]
      });
    ...
    it("should return nulls for unknown names", () => {
      expect(djangoQL.resolveName("gav")).toStrictEqual({
        model: null,
        field: null,
        modelStack: ["core.book"]
      });
      expect(djangoQL.resolveName("author.gav")).toStrictEqual({
        model: null,
        field: null,
        modelStack: ["core.book", "auth.user"]
      });
      expect(djangoQL.resolveName("author.groups.gav")).toStrictEqual({
        model: null,
        field: null,
        modelStack: ["core.book", "auth.user", "auth.group"]
      });
