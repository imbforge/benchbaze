...
lexer.addRule(new RegExp(`startswith${reNotFollowedByName}`), (l) =>
  token("STARTSWITH", l)
);
lexer.addRule(new RegExp(`endswith${reNotFollowedByName}`), (l) =>
  token("ENDSWITH", l)
);
lexer.addRule(new RegExp(`True${reNotFollowedByName}`), (l) =>
  token("TRUE", l)
);
lexer.addRule(new RegExp(`False${reNotFollowedByName}`), (l) =>
  token("FALSE", l)
);
lexer.addRule(new RegExp(`None${reNotFollowedByName}`), (l) =>
  token("NONE", l)
);
...
const DjangoQL = function (options) {
  ...
  if (options.cacheSize) {
    if (
      parseInt(options.cacheSize, 10) !== options.cacheSize ||
      options.cacheSize < 1
    ) {
...
DjangoQL.prototype = {
  ...
  getContext(text, cursorPos) {
    ...
    if (prefix === ")" && !whitespace) {
      ...
      (lastToken.name === "PAREN_L" &&
        (!nextToLastToken || logicalTokens.indexOf(nextToLastToken.name) >= 0))
    ) {
      scope = "field";
      ...
      if (resolvedName.model) {
        ...
        if (
          prefix[0] === '"' &&
          (this.models[model][field].type === "str" ||
      ...
      lastToken &&
      whitespace &&
      ["PAREN_R", "INT_VALUE", "FLOAT_VALUE", "STRING_VALUE"].indexOf(
        lastToken.name
      ) >= 0
    ) {
      scope = "logical";
  ...
  generateSuggestions() {
    ...
    switch (context.scope) {
      case "field":
        this.suggestions = Object.keys(model)
          .filter((f) => {
            const { relation } = model[f];
            if (
              model[f].type === "relation" &&
      ...
      case "comparison":
        ...
        if (field && field.type !== "bool") {
          if (["date", "datetime"].indexOf(field.type) >= 0) {
            suggestions.push(["~", "contains"], ["!~", "does not contain"]);
            snippetAfter = ' "|"';
        ...
        if (field && field.type !== "bool") {
          if (
            ["str", "date", "datetime"].indexOf(field.type) >= 0 ||
            field.options
          ) {
            snippetAfter = ' ("|")';
          } else {
