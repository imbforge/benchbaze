...
$("#id_part_description,#id_supplier_part_no").click(function () {
  ...
  const firstDataFieldName =
    labelFieldName === "part_description"
      ? "supplier_part_no"
      : "part_description";
  ...
  $(`#id_${labelFieldName}`)
    .autocomplete({
      ...
      source: (request, response) => {
        ...
        $.getJSON(
          `/purchasing/order_autocomplete/${labelFieldName}=${request.term.trim().replace("=", "")},${timeStamp}`,
          (data) => {
            response(data);
          }
        );
    ...
    const firstTextElementWidth =
      labelFieldName === "part_description" ? 50 : 16;
    const secondTextElementWidth =
      firstDataFieldName === "part_description" ? 50 : 16;
    ...
    return $("<li>")
      ...
      .append(
        `<span style='margin:0px 5px; white-space: nowrap; padding:3px;'>${displayText}</span>`
      )
      .appendTo(ul);
  };
});
